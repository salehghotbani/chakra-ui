...
Set the `--action-bar-offset` CSS variable on `ActionBar.Positioner` to leave
space for fixed UI such as a bottom navigation bar.

<ExampleTabs name="action-bar-with-custom-offset" />
