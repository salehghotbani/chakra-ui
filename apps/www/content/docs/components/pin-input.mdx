...
Pin Input automatically sets `enterKeyHint="next"` on intermediate inputs and
`enterKeyHint="done"` on the last input. Supported mobile keyboards use these
hints to display the appropriate action key.

<ExampleTabs name="pin-input-with-enter-key-hint" />
