...
### Check Whether an Overlay Exists

Use the `.has` method before opening an overlay to prevent creating another
overlay with the same id.

<ExampleTabs name="overlay-with-has-check" />
...
Returns whether an overlay with the given id exists.
