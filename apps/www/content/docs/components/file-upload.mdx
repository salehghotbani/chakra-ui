...
Selecting the same file more than once rejects the duplicate with the
`FILE_EXISTS` error. Use `onFileReject` to display feedback for rejected files.

<ExampleTabs name="file-upload-duplicate-rejection" />
