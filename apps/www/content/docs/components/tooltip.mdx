...
When the pointer moves directly between tooltip triggers, the next content
receives the `data-instant` attribute. Use it to skip the enter animation when
switching between related controls.

<ExampleTabs name="tooltip-with-instant-transitions" />
